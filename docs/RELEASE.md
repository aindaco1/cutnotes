...
./scripts/release/release.sh 1.0.6
```
...
Push an annotated `v1.0.6` tag only after local acceptance. `.github/workflows/release.yml` repeats the release pipeline using repository secrets and uploads:

- `CutNotes-1.0.6-arm64.dmg`
- `appcast.xml`
- `SHA256SUMS`
